...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=70.0,
        help="Expected max gripper stroke in mm for mobile-aloha-style normalization. Common values: 70 or 100.",
    )
    parser.add_argument(
        "--joint-margin-deg",
        type=float,
        default=3.0,
        help="Allowed arm-joint range slack in degrees before flagging an error.",
    )
    parser.add_argument(
        "--gripper-margin-mm",
        type=float,
        default=5.0,
        help="Allowed gripper range slack in mm before flagging an error.",
    )
    parser.add_argument(
        "--gripper-norm-margin",
        type=float,
...
def main() -> None:
    ...
    projected_state = _project_to_mobile_aloha(state_values, state_names, args.gripper_max_mm)
    projected_action = _project_to_mobile_aloha(action_values, action_names, args.gripper_max_mm)
    ...
    print(
        f"- gripper_max_mm: {args.gripper_max_mm:.1f} "
