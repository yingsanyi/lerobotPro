    ...
    def _read_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                failure_count += 1
                # Avoid flooding logs while preserving visibility into recurrent issues.
                if failure_count <= 3 or failure_count % 5 == 0:
                    logger.warning(
                ...
                if failure_count >= max_failures_before_reopen:
                    # Try in-loop recovery instead of killing the read thread.
                    # This improves resilience to transient USB/V4L2 glitches.
                    reopened = False
                    try:
                        if self.videocapture is not None:
                            self.videocapture.release()
                        ...
                        if self.videocapture is not None and self.videocapture.isOpened():
                            try:
                                ...
                                # Continue even if strict setting checks fail after recovery.
                                logger.warning(
                                    f"{self} reopened camera but failed to fully re-apply capture settings: {cfg_exc}"
                                )
                            reopened = True
                            logger.warning(f"{self} recovered read loop by reopening VideoCapture.")
                        else:
                            if self.videocapture is not None:
                                self.videocapture.release()
                                self.videocapture = None
                    except Exception as reopen_exc:
                        logger.warning(f"{self} failed to reopen VideoCapture: {reopen_exc}")
