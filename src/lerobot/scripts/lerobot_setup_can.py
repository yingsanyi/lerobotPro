...
def test_motor(bus, motor_id: int, timeout: float, use_fd: bool):
    ...
    expected_recv_id = (motor_id + 0x10) & 0x7FF
    responses = []
    start_time = time.time()

    while time.time() - start_time < timeout:
        msg = bus.recv(timeout=0.1)
        if msg and msg.arbitration_id == expected_recv_id:
...
def test_interface(cfg: CANSetupConfig, interface: str):
    ...
    try:
        # On some setups (e.g. integrated chains), CAN traffic may be continuous.
        # Drain with bounds to avoid hanging forever.
        ...
        while True:
            ...
            if drained >= max_drain_msgs or (time.time() - drain_start) >= max_drain_s:
                print(f"  [info] stop pre-test drain early (drained={drained}, elapsed={time.time() - drain_start:.3f}s)")
                break
        ...
        if all(not r.get("found", False) for r in results.values()):
            ...
            while time.time() - sample_start < 0.2:
                ...
                if msg is not None:
                    seen_ids.add(msg.arbitration_id)
            if seen_ids:
                pretty_ids = ", ".join(f"0x{i:03X}" for i in sorted(seen_ids)[:24])
                extra = "" if len(seen_ids) <= 24 else f", ... (+{len(seen_ids) - 24} more)"
                print(f"  [info] Bus is active but no Damiao handshake responses. Seen IDs: {pretty_ids}{extra}")
    finally:
        bus.shutdown()
