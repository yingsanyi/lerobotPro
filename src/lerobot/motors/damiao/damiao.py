...
class DamiaoMotorsBus(MotorsBusBase):
    ...
    def _handshake(self) -> None:
        ...
        # Important: on some setups the bus may continuously stream frames (e.g. periodic status).
        # Draining until `recv()` returns None can hang forever. Instead, drain best-effort with bounds.
        drain_start_s = time.time()
        drained = 0
        max_drain_s = 0.5
        max_drain_msgs = 500
        while True:
            msg = self.canbus.recv(timeout=0.01)
            if msg is None:
                break
            drained += 1
            if drained >= max_drain_msgs or (time.time() - drain_start_s) >= max_drain_s:
                logger.debug(
                    "Stopping handshake drain early (drained=%s, elapsed=%.3fs).",
                    drained,
                    time.time() - drain_start_s,
                )
                break
