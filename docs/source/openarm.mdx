...
If your Songling ALOHA setup is wired as bimanual leader/follower over CAN, you can
reuse the same OpenArm stack directly with a config file profile.

The Songling example keeps a single maintained profile config under:

`examples/songling_aloha/teleop.yaml`
...
Current supported bring-up path:
...
`teleop.yaml` is also the file updated by `lerobot-find-port` and `lerobot-find-cameras` for CAN and OpenCV mapping. Dedicated Songling record/replay flows remain future work until the protocol adapter lands in main LeRobot.
